<template>
  <v-app>
    <v-app-bar app color="white" light elevation="0">
      ...
      <v-card flat>
        <v-tabs class="d-none d-sm-flex" color="deep-purple-accent-4" align-tabs="end">
          <v-tab v-for="(menu, index) in menus" :key="`menu_${index}`" :value="menu" :to="menu.route">
            {{ menu.titulo }}
          </v-tab>
          ...
          <v-tab v-else  @click="navigateToAdminPanel">
    ...
    <v-navigation-drawer v-model="drawer" app mobile-breakpoint="768" disable-resize-watcher>
